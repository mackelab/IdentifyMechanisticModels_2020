...
class HH:
    ...
    def sim_time(self, dt, t, I):
        ...
        for i in range(1, t.shape[0]):
            tau_V_inv = ( (m[i-1]**3)*gbar_Na*h[i-1]+(n[i-1]**4)*gbar_K+g_leak+gbar_M*p[i-1]
                         +gbar_L*(q[i-1]**2)*r[i-1] + gbar_T*(s_inf(V[i-1])**2)*u[i-1] )/C
            V_inf = ( (m[i-1]**3)*gbar_Na*h[i-1]*E_Na+(n[i-1]**4)*gbar_K*E_K+g_leak*E_leak+gbar_M*p[i-1]*E_K
                     +gbar_L*(q[i-1]**2)*r[i-1]*E_Ca + gbar_T*(s_inf(V[i-1])**2)*u[i-1]*E_Ca
                    +I[i-1]+nois_fact*self.rng.randn()/(tstep**0.5) )/(tau_V_inv*C)
